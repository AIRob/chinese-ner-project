...
import codecs 
from bilstm_crf_gpu import BiLSTM_CRF
from calc_result import calculate
...
model = BiLSTM_CRF(len(word2id)+1, tag2id, EMBEDDING_DIM, HIDDEN_DIM).to(device)
...
for epoch in range(EPOCHS):
    ...
    for sentence, tags in zip(x_train,y_train):
        ...
        tags = torch.tensor([tag2id[t] for t in tags], dtype=torch.long, device=device)
